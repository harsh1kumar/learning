/*
 * Sorted Linked List Implementation using Arrays
 */
...
{
	int node;
	for ( node = start; node != -1 && ll_data[node] < data; node = ll_link[node])
		;
